...
class Location:
    ...
    async def get_timezone(self):
        self.bot.config.timezonedb
        url = f("http://api.timezonedb.com/v2.1/get-time-zone?key={key}"
                "&format=json&by=position&lat={self.latitude}&lng={self.longitude}")
...
class AuthorInfo:
    ...
    def single_setter(self, key, value):
        q = '''REPLACE INTO userinfo VALUES (?, ?, ?, ?) WHERE user = (?) AND field = (?); '''
        self.c.execute(q, (self.user_id, str(user), key, value, self.user_id, key))
        self.conn.commit()

...
class Copilot(commands.Cog):
    ...
    async def clai(self, ctx, *, ask: str):
        ...
        if not await _check_bot_admin(ctx):
            if not await self.ai_cache.acl_is_allowed(ctx.guild.id, ctx.author.id):
...
{ask}
    ...
    async def claiconfig(self, ctx, key: str = None, *, value: str = None):
        ...
        # Cross-channel support: !claiconfig #channel key value
        import re as _re
        ch_match = _re.match(r'<#(\d+)>', key or '')
        if ch_match:
            channel_id = int(ch_match.group(1))
            # Shift: value contains "key value" or just "key"
            if value:
    ...
    async def glmconfig(self, ctx, key: str = None, *, value: str = None):
        ...
        # Cross-channel support: !glmconfig #channel key value
        import re as _re
        ch_match = _re.match(r'<#(\d+)>', key or '')
        if ch_match:
            channel_id = int(ch_match.group(1))

...
class Copilot(commands.Cog):
    ...
    def __init__(self, bot):
        ...
        self.context_gatherer = ContextGatherer(bot, self.ai_cache)
    ...
    async def clai(self, ctx, *, ask: str):
        ...
        async with ctx.channel.typing():
            ...
            if use_context:
                channel_context = await self.context_gatherer._build_compacted_context(ctx, settings, token, base_url)
            else:
                channel_context = ""
            if channel_context:
                ...
                if "[Conversation summary" in channel_context and "Recent channel conversation:" in channel_context:
                    ...
                    if show_debug:
                        if self.context_gatherer._last_compaction:
                            debug_parts.append(f"⟳{self.context_gatherer._last_compaction}")
                        ...
                        if hasattr(self.context_gatherer, '_overflow_tokens'):
                            debug_parts.append(f"overflow={self.context_gatherer._overflow_tokens}/{self.context_gatherer._recompact_threshold}")
                else:
            ...
            if use_context:
                user_context = await self.context_gatherer.gather_user_context(ctx)
                if user_context:
...
- When addressing users, use their display name (it will be auto-converted to a mention)"""
    ...
    async def _rebuild_channel_cache(self, ctx, channel, settings, token, base_url) -> str:
        ...
        if older_tokens > self.MAX_COMPACTION_INPUT:
            older_msgs = self.context_gatherer._trim_messages_to_budget(older_msgs, bot_user_id, self.MAX_COMPACTION_INPUT)
            older_text = self.context_gatherer._format_messages(older_msgs, bot_user_id)
